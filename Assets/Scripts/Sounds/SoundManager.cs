...
namespace Infrastructure {
	...
	internal class SoundManager : ISoundManager {
		...
		[Inject]
		public SoundManager(SoundConfig soundConfig, IRandomController randomController, IViewsFactory viewsFactory) {
		...
		public void Init() {
			...
			void InitPlayer() {
				...
				if (PlayerPrefs.HasKey(TextConstants.MUSIC_VOLUME_PREFS_KEY)) {
					SetMusicVolume(PlayerPrefs.GetFloat(TextConstants.MUSIC_VOLUME_PREFS_KEY));
				}
				if (PlayerPrefs.HasKey(TextConstants.SOUND_VOLUME_PREFS_KEY)) {
					SetSoundVolume(PlayerPrefs.GetFloat(TextConstants.SOUND_VOLUME_PREFS_KEY));
				}
			}
