...
namespace Units {
	...
	internal sealed class SkillsManager : ISkillsManager {
		...
		[Inject]
		public SkillsManager(IWeaponsManager weaponsManager, IRandomManager randomManager, IMissionUiController missionUiController
		...
		private void PrepareSkillsForUpgrade(int playerLevel) {
			...
			for (var i = 0; i < Math.Min(ChoosingSkillsAmount, _weaponsManager.UpgradableWeaponTypes.Count); i++) {
				do {
					randomSkillIndex = _randomManager.GetRandomExcludingMax(_weaponsManager.UpgradableWeaponTypes.Count);
				} while (randomSkillIndexes.Contains(randomSkillIndex));
				randomSkillIndexes.Add(randomSkillIndex);
			}
			var randomSkillTypes = randomSkillIndexes.Select(skillIndex => _weaponsManager.UpgradableWeaponTypes[skillIndex]).ToArray();
			var choosingSkills = new ActualSkillInfo[randomSkillTypes.Length];
			for (var i = 0; i < randomSkillTypes.Length; i++) {
				choosingSkills[i] = new ActualSkillInfo(randomSkillTypes[i], _weaponsManager.GetCurrentLevelOfWeapon(randomSkillTypes[i]) + 1);
