	...
	internal sealed class LobbyPanelView : MonoBehaviour {
		...
		public void AddRoom(RoomInfo room) {
			...
			if (!_cachedRoomItemViews.ContainsKey(roomName)) {
				...
				roomItem.RoomName.color = room.IsOpen ? _mainMenuConfig.UnlockedRoomLabelColor : _mainMenuConfig.LockedRoomLabelColor;
		...
		private void UpdatePrivateRoomButtonsInteractivity(string privateRoomName) {
			_createPrivateRoomButton.interactable = !_uiIsBlocked && !string.IsNullOrEmpty(privateRoomName) && !_cachedRoomItemViews.ContainsKey(privateRoomName);
