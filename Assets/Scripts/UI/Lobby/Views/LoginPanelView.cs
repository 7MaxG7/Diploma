...
namespace Infrastructure {
	...
	internal class LoginPanelView : MonoBehaviour {
		...
		public void Init(MainMenuConfig mainMenuConfig) {
			...
			_confirmButton.onClick.AddListener(() => OnLoginConfirmClick?.Invoke(_usernameInputField.text, _passwordInputField.text, _emailInputField.text));
		...
		private void UpdateConfirmButtonInteractivity() {
			_confirmButton.interactable = !string.IsNullOrEmpty(_usernameInputField.text)
			                              ...
			                              && (_isNewAccount && !string.IsNullOrEmpty(_emailInputField.text) || !_isNewAccount);
		}
	}
