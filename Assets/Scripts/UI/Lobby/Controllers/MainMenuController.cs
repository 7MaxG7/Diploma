...
namespace Infrastructure {
	...
	internal sealed class MainMenuController : IMainMenuController {
		...
		[Inject]
		public MainMenuController(IMissionResultManager missionResultManager, IPermanentUiController permanentUiController
				, IViewsFactory viewsFactory, IPhotonManager photonManager, IPlayfabManager playfabManager, MainMenuConfig mainMenuConfig) {
		...
		public void SetupMainMenu() {
			...
			void InitLoginPanel() {
				_loginPanelController = new LoginPanelController(_playfabManager, _mainMenuView.LoginPanelView, _mainMenuConfig);
				_loginPanelController.Init();
				_loginPanelController.OnUserLoginSuccess += SetUser;
			}

			void InitLobbyPanel() {
				_lobbyScreenController = new LobbyScreenController(_photonManager, _mainMenuView.LobbyScreenView, _mainMenuConfig, _permanentUiController);
		...
		private void SetupScoresLable() {
			...
			_playfabManager.GetData(_playfabId, ParseData, LogError, Constants.WINS_AMOUNT_PLAYFAB_KEY, Constants.KILLS_AMOUNT_PLAYFAB_KEY);
			
			void ParseData(Dictionary<string, string> data) {
				var winsScores = 0;
				var killsScores = 0;
				if (data.ContainsKey(Constants.WINS_AMOUNT_PLAYFAB_KEY) && int.TryParse(data[Constants.WINS_AMOUNT_PLAYFAB_KEY], out var winsAmount)) {
					_missionResultManager.SetWinsAmount(winsAmount);
					winsScores = winsAmount * _mainMenuConfig.ScorePerWin;
				}
				if (data.ContainsKey(Constants.KILLS_AMOUNT_PLAYFAB_KEY) && int.TryParse(data[Constants.KILLS_AMOUNT_PLAYFAB_KEY], out var killsAmount)) {
