...
namespace Services
{
    internal sealed class MissionMapManager : IMissionMapManager
    {
        ...
        [Inject]
        public MissionMapManager(IViewsFactory viewsFactory, IControllersHolder controllersHolder,
        ...
        public void OnUpdate(float deltaTime)
        {
            ...
            foreach (var groundItem in _groundItems)
            {
                ...
                {
                    groundItem.RelocateBy(
                        new Vector3(_groundItemSize.x * Constants.GROUND_ITEM_RELOCATION_DISTANCE_RATE, 0, 0));
                }
                ...
                {
                    groundItem.RelocateBy(new Vector3(0,
                        _groundItemSize.y * Constants.GROUND_ITEM_RELOCATION_DISTANCE_RATE, 0));
                }
            ...
            {
                return ItemIsTooFar(_playerTransform.position.x, itemPositionX,
                    _groundItemSize.x * Constants.MAX_PLAYER_TO_GROUND_ITEM_DISTANCE_RATE);
            }

            bool ItemIsTooRight(float itemPositionX)
            {
                return ItemIsTooFar(itemPositionX, _playerTransform.position.x,
                    _groundItemSize.x * Constants.MAX_PLAYER_TO_GROUND_ITEM_DISTANCE_RATE);
            }

            bool ItemIsTooLow(float itemPositionY)
            {
                return ItemIsTooFar(_playerTransform.position.y, itemPositionY,
                    _groundItemSize.y * Constants.MAX_PLAYER_TO_GROUND_ITEM_DISTANCE_RATE);
            }

            bool ItemIsTooHigh(float itemPositionY)
            {
                return ItemIsTooFar(itemPositionY, _playerTransform.position.y,
                    _groundItemSize.y * Constants.MAX_PLAYER_TO_GROUND_ITEM_DISTANCE_RATE);
            }
        ...
        public void Init(Transform playerTransform, Vector2 groundItemSize, out IEnumerable<IView> groundItems)
        {
            ...
            {
                _groundItems = new GroundItemView[Constants.HORIZONTAL_GROUND_ITEMS_AMOUNT *
                                                  Constants.VERTICAL_GROUND_ITEMS_AMOUNT];
                ...
                for (var i = 0; i < Constants.HORIZONTAL_GROUND_ITEMS_AMOUNT; i++)
                {
                    for (var j = 0; j < Constants.VERTICAL_GROUND_ITEMS_AMOUNT; j++)
                    {
                        ...
                        var groundPosition = playerTransform.position +
                                             new Vector3(_groundItemSize.x * (i - 1), _groundItemSize.y * (j - 1));
