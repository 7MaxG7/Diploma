...
namespace Infrastructure
{
    internal sealed class MissionResultManager : IMissionResultManager, IInRoomCallbacks
    {
        ...
        [Inject]
        public MissionResultManager(IPhotonManager photonManager, IPlayfabManager playfabManager,
            IPermanentUiController permanentUiController, MissionConfig missionConfig)
        {
            ...
            if (damageInfo.Damager == _player)
            {
                _playfabManager.SetData(
                    new Dictionary<string, string>
                        { { Constants.KILLS_AMOUNT_PLAYFAB_KEY, (++_currentKillsAmount).ToString() } }
                    , null
                    , OnErrorResult
                );
            }
        ...
        private void WinGame()
        {
            _playfabManager.SetData(
                new Dictionary<string, string>
                    { { Constants.WINS_AMOUNT_PLAYFAB_KEY, (++_currentWinsAmount).ToString() } }
                , null
                , OnErrorResult
            );
