...
namespace Services
{
    internal sealed class MonstersSpawner : IMonstersSpawner
    {
        ...
        public MonstersSpawner(IMonstersMoveManager monstersMoveManager, IRandomManager randomManager
            , ICameraManager cameraManager, IUnitsPool unitsPool, MonstersConfig monstersConfig,
        ...
        private IUnit SpawnMonster()
        {
            ...
            Vector2 GenerateSpawnPosition()
            {
                ...
                bottomLeftCorner -= new Vector3(_monstersConfig.HorizontalSpawnFromScreenOffset,
                    _monstersConfig.VerticalSpawnFromScreenOffset);
                var topRightCorner = _mainCamera.ViewportToWorldPoint(new Vector3(1, 1, 0));
                topRightCorner += new Vector3(_monstersConfig.HorizontalSpawnFromScreenOffset,
                    _monstersConfig.VerticalSpawnFromScreenOffset);

                switch (spawnSide)
                {
                    case ScreenSide.Left:
                        return new Vector2(bottomLeftCorner.x,
                            _random.GetRandomIncludingMax((int)topRightCorner.y, (int)bottomLeftCorner.y));
                    case ScreenSide.Bottom:
                        return new Vector2(
                            _random.GetRandomIncludingMax((int)topRightCorner.x, (int)bottomLeftCorner.x),
                            bottomLeftCorner.y);
                    case ScreenSide.Right:
                        return new Vector2(topRightCorner.x,
                            _random.GetRandomIncludingMax((int)topRightCorner.y, (int)bottomLeftCorner.y));
                    case ScreenSide.Top:
                        return new Vector2(
                            _random.GetRandomIncludingMax((int)topRightCorner.x, (int)bottomLeftCorner.x),
